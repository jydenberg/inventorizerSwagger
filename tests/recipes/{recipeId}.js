...
/**
 * Test for /recipes/{recipeId}
 */
Test('/recipes/{recipeId}', function (t) {
    ...
    Parser.validate(apiPath, function (err, api) {
         ...
         * responses: 200, 400
         */
        t.test('test searchRecipes get operation', function (t) {
            Mockgen().requests({
                ...
                request.end(function (err, res) {
                    ...
                    var validate = Validator(api.paths['/recipes/{recipeId}']['get']['responses']['200']['schema']);
         ...
         * responses: 200, 400
         */
        t.test('test editRecipe put operation', function (t) {
            Mockgen().requests({
                path: '/recipes/{recipeId}',
                operation: 'put'
            }, function (err, mock) {
                ...
                request.end(function (err, res) {
                    ...
                    t.ok(res.statusCode === 200, 'Ok response status');
                    var Validator = require('is-my-json-valid');
                    var validate = Validator(api.paths['/recipes/{recipeId}']['put']['responses']['200']['schema']);
                    var response = res.body;
                    if (Object.keys(response).length <= 0) {
                        response = res.text;
                    }
        ...
        t.test('test deleteRecipe delete operation', function (t) {
            Mockgen().requests({
                ...
                request.end(function (err, res) {
                    ...
                    t.ok(res.statusCode === 200, 'Ok response status');
                    var Validator = require('is-my-json-valid');
                    var validate = Validator(api.paths['/recipes/{recipeId}']['delete']['responses']['200']['schema']);
                    var response = res.body;
                    if (Object.keys(response).length <= 0) {
                        response = res.text;
                    }
