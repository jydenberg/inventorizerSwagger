...
available inventory in the system
         ...
         * responses: 200, 400
         */
        t.test('test listRecipes get operation', function (t) {
            Mockgen().requests({
                path: '/recipes',
                operation: 'get'
            }, function (err, mock) {
                ...
                request.end(function (err, res) {
                    ...
                    var validate = Validator(api.paths['/recipes']['get']['responses']['200']['schema']);
         ...
         */
        t.test('test addRecipe post operation', function (t) {
            Mockgen().requests({
                path: '/recipes',
                operation: 'post'
            }, function (err, mock) {
                ...
                //Mock request Path templates({}) are resolved using path parameters
                request = Request(App)
                    .post('/yjosh/ElevatedV3/1.0.0' + mock.request.path);
                if (mock.request.body) {
                    //Send the request body
                    request = request.send(mock.request.body);
                } else if (mock.request.formData){
                ...
                request.end(function (err, res) {
                    ...
                    var Validator = require('is-my-json-valid');
                    var validate = Validator(api.paths['/recipes']['post']['responses']['201']['schema']);
                    var response = res.body;
                    if (Object.keys(response).length <= 0) {
                        response = res.text;
                    }
